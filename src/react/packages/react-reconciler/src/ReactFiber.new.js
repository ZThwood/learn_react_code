...
import type { OffscreenProps, OffscreenInstance } from './ReactFiberOffscreenComponent';
...
function FiberNode(tag: WorkTag, pendingProps: mixed, key: null | string, mode: TypeOfMode) {
    /** 基础属性 **/
    // 组件对象类型
    this.tag = tag; // 标识 Fiber 类型
    this.key = key; // 用于列表渲染的优化标识
    this.elementType = null; // React元素类型（函数、类或字符串标签）
    // 元素类型
    this.type = null; // 与elementType类似，但可能不同（如ForwardRef）
    /**
     * 实际实例
     * - DOM节点（对于HostComponent）
     * - 类组件实例（对于ClassComponent）
     * - null（对于FunctionComponent）
     */
    this.stateNode = null;
    ...
    /**状态相关属性 */
    // 组件初始属性
    this.pendingProps = pendingProps; // 新传入的props（尚未处理）

    // 更新后的props
    this.memoizedProps = null; // 上一次渲染使用的props
    ...
    */
    ...
    this.dependencies = null; // 上下文依赖集合
     ...
     * flags副作用标识
          ...
          Update：需要更新
          Deletion：需要删除
          Passive：有useEffect副作用
     */
    this.subtreeFlags = NoFlags; // 子树中的副作用标识
    this.deletions = null;

    this.lanes = NoLanes; // 优先级车道
    this.childLanes = NoLanes; // 子节点的优先级车道
    /**
     * 指向workInProgress树或current树的对应节点
     * 比如
...
export function createFiberFromSuspense(pendingProps: any, mode: TypeOfMode, lanes: Lanes, key: null | string) {
...
export function createFiberFromSuspenseList(pendingProps: any, mode: TypeOfMode, lanes: Lanes, key: null | string) {
...
export function createFiberFromOffscreen(pendingProps: OffscreenProps, mode: TypeOfMode, lanes: Lanes, key: null | string) {
...
export function createFiberFromLegacyHidden(pendingProps: OffscreenProps, mode: TypeOfMode, lanes: Lanes, key: null | string) {
...
export function createFiberFromCache(pendingProps: any, mode: TypeOfMode, lanes: Lanes, key: null | string) {
...
export function createFiberFromTracingMarker(pendingProps: any, mode: TypeOfMode, lanes: Lanes, key: null | string) {
